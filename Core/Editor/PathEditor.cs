...
namespace PathCreationEditor {
    ...
    public class PathEditor : Editor {
        ...
        bool useRaycast = true, useCustomLayer = false;
        int raycastLayer = 0;
        public bool UseRaycast { get { return useRaycast; } }
        public bool UseCustomLayer { get { return useCustomLayer; } }
        public int RaycastLayer { get { return raycastLayer; } }
        void DrawBezierPathInspector () {
            using (var check = new EditorGUI.ChangeCheckScope ()) {

                EditorGUILayout.HelpBox("After selecting an anchor point, press 'M' key to focus on it", MessageType.Info);
                // Path options:
                ...
                if (data.showPathOptions) {
                    ...
                    useRaycast = EditorGUILayout.Toggle("Use Raycast? ", useRaycast);
                    useCustomLayer = EditorGUILayout.Toggle("Use Custom Layer? ", useCustomLayer);
                    if (useCustomLayer)
                    {
                        EditorGUI.indentLevel++;
                        LayerMask tempMask = EditorGUILayout.MaskField(InternalEditorUtility.LayerMaskToConcatenatedLayersMask(raycastLayer), 
                            InternalEditorUtility.layers);
                        raycastLayer = InternalEditorUtility.ConcatenatedLayersMaskToLayerMask(tempMask);
                        EditorGUI.indentLevel--;
                    }
        ...
        void ProcessBezierPathInput (Event e) {
            ...
            if (mouseOverHandleIndex == -1) {
                if (e.type == EventType.MouseDown && e.button == 0 && e.shift) {
                    ...
                    if (selectedSegmentIndex != -1 && selectedSegmentIndex < bezierPath.NumSegments) {
                        Vector3 newPathPoint = Vector3.zero;
                        if (useRaycast)
                        {
                            var success = false;
                            var hitPt = MouseUtility.GetRaycastHitPoint(ref success, useCustomLayer, raycastLayer);
                            if (success)
                            {
                                newPathPoint = hitPt;
                            }
                            ...
                            {
                                newPathPoint = pathMouseInfo.closestWorldPointToMouse;
                            }
                        }
                        else
                        {
                    ...
                    // If path is not a closed loop, add new point on to the end of the path
                        ...
                        Vector3 newPathPoint = MouseUtility.GetMouseWorldPosition(bezierPath.Space, dstCamToEndpoint, useRaycast, useCustomLayer, raycastLayer);
                        newPathPoint = MathUtility.InverseTransformPoint (newPathPoint, creator.transform, bezierPath.Space);
            ...
            {
                var isAnchorPoint = handleIndexToDisplayAsTransform % 3 == 0;
                if (isAnchorPoint)
                {
        ...
        void DrawHandle (int i) {
            ...
            handlePosition = PathHandle.DrawHandle (handlePosition, bezierPath.Space, isInteractive, handleSize, cap, handleColours, out handleInputType, i, this);
